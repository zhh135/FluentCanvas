...
[四、感谢](#thank)  
[五、图片](#pictu)
## FluentCanvas 是什么? <span id='intro'></span>
FluentCanvas 画板是一款轻量级画板软件，其针对希沃白板设备进行了特别优化，与预装的“希沃白板 5”软件相比，启动速度大幅度提升（提升5-10 倍），系统资源占用更小，使用体验更佳。  
...
* **保存墨迹**：默认保存至 `文档\FluentCanvas Strokes`
* **截图**：任意模式模式下（包括鼠标）下点击相机图标截图并自动保存至 `图片\FluentCanvas Screenshots`，可在设置中开启“截图时自动保存墨迹”
* **幻灯片自动保存墨迹**：默认保存至 `文档\FluentCanvas Strokes`
* **墨迹回放**：从头自动书写一遍屏中墨迹
...
3. 如果需要关闭FluentCanvas画板，可在设置中找到关闭按钮。
...
特别感谢 [yuwenhui2020](https://github.com/yuwenhui2020) 为 ` FluentCanvas 使用说明` 做出的贡献！
