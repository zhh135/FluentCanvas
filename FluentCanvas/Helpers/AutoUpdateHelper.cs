...
namespace FluentCanvas.Helpers
{
    internal class AutoUpdateHelper
    {
        ...
        public static async Task<string> CheckForUpdates()
        {
            if (App.IsPackaged)
            {
                LogHelper.WriteLogToFile("AutoUpdate | MSIX servicing is managed by the package installer.");
                return null;
            }
        ...
        private static string updatesFolderPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "FluentCanvas", "AutoUpdate");
        ...
        public static async Task<bool> DownloadSetupFileAndSaveStatus(string version)
        {
            ...
            try
            {
                ...
                string setupFileName = $"FluentCanvas.V{version}.Setup.exe";
